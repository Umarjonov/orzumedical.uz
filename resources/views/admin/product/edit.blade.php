...
@section('content')
            ...
            <div class="header-body">
                <div class="row align-items-center py-4">
                    <div class="col-lg-6 col-7">
                        <h6 class="h2 text-white d-inline-block mb-0">@lang('global.edit') Продукт</h6>
    ...
    <div class="container-fluid mt--6 mb-4">
        <form action="{{ route('product.update',$product->id) }}" method="post" enctype="multipart/form-data" id="form_store">
            @csrf
            @method('PUT')
            <div class="row">
                <div class="col-lg-4">
                            ...
                            <div class="d-flex flex-wrap mt-3" id="div_gallery" style="gap: 8px">
                                ...
                                @if($gallery)
                                    @foreach($gallery as $image)
                                        <img class="{{$loop->first?'w-100 shadow-lg mt-3 card_image':'cards_img'}}" src="{{asset('uploads/products/'.$image)}}" alt="{{$image}}">
                                    @endforeach
                                @endif
                            </div>
                        </div>
                            ...
                            <div class="row">
                                <div class="col-sm-6">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input class="form-control" type="text" name="articul" value="{{old('articul',$product->articul)}}">
                                    </div>
                                    <div class="form-group">
                                        <label>Вес (кг)</label>
                                        <input name="weight" class="form-control" type="number" value="{{old('weight',$product->weight)}}">
                                    </div>
                                    <div class="form-group">
                                        ...
                                        <select class="form-control {{ $errors->has('catalog_id') ? "is-invalid":"" }}" name="catalog_id" id="catalog_id" data-placeholder="@lang('pleaseSelect')">
                                            ...
                                            @foreach($catalogs as $catalog)
                                                <option value="{{ $catalog->id }}" {{$catalog->id==$product->catalog_id?'selected':''}}>{{ $catalog->id.'-'.$catalog->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="form-group">
                                        ...
                                        <select class="form-control {{ $errors->has('brand_id') ? "is-invalid":"" }}" name="brand_id" id="brand_id" data-placeholder="@lang('pleaseSelect')">
                                            ...
                                            @foreach($brands as $brand)
                                                <option value="{{ $brand->id }}" {{$brand->id==$product->brand_id?'selected':''}}>{{ $brand->id.'-'.$brand->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                <div class="col-sm-6">
                                    <div class="form-group">
                                        ...
                                        <textarea name="title" id="title" class="form-control" cols="30" rows="2">{{old('title',$product->title)}}</textarea>
                                    </div>
                                    <div class="form-group">
                                        <label>Описание</label>
                                        <textarea name="description" id="description" class="form-control" cols="30" rows="6">{{old('information',$product->information)}}</textarea>
                                    </div>
                                    <div class="form-group">
                                        <label>Информация</label>
                                        <textarea name="information" id="information" class="form-control" cols="30" rows="6">{{old('information',$product->information)}}</textarea>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div class="col-sm-4">
                    <div class="card h-100">
                        <div class="card-body">
                            ...
                            <label>Title</label>
                            <input name="seo_title" class="form-control" type="text" value="{{old('seo_title',$product->seo_title)}}">
                            <label class="mt-3">Keywords</label>
                            <input name="seo_keywords" class="form-control" type="text" value="{{old('seo_keywords',$product->seo_keywords)}}">
                            <label class="mt-3">Description</label>
                            <textarea name="seo_description" id="seo_description" class="form-control" cols="30" rows="2">{{old('seo_description',$product->seo_description)}}</textarea>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="card-body">
                            ...
                            <div class="row">
                                <div class="col-4 form-group">
                                    ...
                                    <input name="count" class="form-control" type="text" value="{{old('count',$product->count)}}">
                                </div>
                                ...
                                <div class="col-3 form-group">
                                    ...
                                    <select class="form-control" name="currency" id="currency" data-placeholder="@lang('pleaseSelect')">
                                        <option value="1" {{$product->currency==1?'selected':''}}>SO'M</option>
                                        <option value="2" {{$product->currency==2?'selected':''}}>USD</option>
                                    </select>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="card-body">
                            ...
                            <div class="row">
                                <div class="col-4 form-group">
                                    ...
                                    <input name="telegram" class="form-control" type="text" value="{{old('telegram',$product->telegram)}}">
                                </div>
                                <div class="col-4 form-group">
                                    <label>Ссылка на Фейсбук</label>
                                    <input name="facebook" class="form-control" type="text" value="{{old('facebook',$product->facebook)}}">
                                </div>
                                <div class="col-4 form-group">
                                    <label>Ссылка на Инстаграм</label>
                                    <input name="instagram" class="form-control" type="text" value="{{old('instagram',$product->instagram)}}">
                                </div>
                            </div>
                        </div>
                    </div>
